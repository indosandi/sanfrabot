...
class TeleBotMediator:
    ...
    """
    def __init__(self, token,host,user,passwd,vhost,port,skip_pending=True,):
        """
    ...
    def __non_threaded_polling(self, none_stop=False, interval=0, timeout=3):
        ...
        while not self.__stop_polling.wait(interval):
            try:
                self.__retrieve_updates_to_rbt(timeout,channel)
                error_interval = .25
...
class TeleBot:
    """ This is TeleBot Class
        ...
        """
    def __init__(self, token,host,user,passwd,vhost,port,skip_pending=True,):
    # def __init__(self, token, threaded=False, skip_pending=False, num_thread=2):
        """
        :param token: bot API token
        :return: Telebot object.
        """
    ...
    def __non_threaded_polling(self, none_stop=False, interval=0, timeout=3):
        ...
        channel = connection.channel()
        channel.queue_declare(queue='bot')
        channel.basic_consume(self.callback,queue='bot',no_ack=True)
